...
var SUPABASE_URL = process.env.VITE_SUPABASE_URL || "https://ldivsmoqttphhuwdjszh.supabase.co";
var SUPABASE_ANON_KEY = process.env.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxkaXZzbW9xdHRwaGh1d2Rqc3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg1OTUwODAsImV4cCI6MjEwNDE3MTA4MH0.vHKy_SbZkVJOjapYq_BJLO5XRMf14OvViZ_-8nfA1RY";
var supabaseServer = null;
try {
  supabaseServer = (0, import_supabase_js.createClient)(SUPABASE_URL, SUPABASE_ANON_KEY);
} catch (err) {
...
async function checkAndExpireRequestsServerSide() {
  ...
  try {
    ...
    const { data: expiredRentals, error: rErr } = await supabaseServer.from("rental_requests").update({ status: "cancelled" }).eq("status", "pending").lte("created_at", fortyEightHoursAgo).select("id, listing_id, renter_id, owner_id");
    if (rErr && !rErr.message?.includes("does not exist")) {
      console.warn("[Server] Auto-expire rental_requests warning:", rErr.message);
    }
    const { data: expiredServices, error: sErr } = await supabaseServer.from("service_bookings").update({ status: "cancelled" }).eq("status", "pending").lte("created_at", fortyEightHoursAgo).select("id, listing_id, customer_id, provider_id");
...
async function startServer() {
  ...
  const AUTHORIZED_ADMIN_EMAILS = /* @__PURE__ */ new Set([
  ...
  const requireAdminAuth = (req, res, next) => {
    const rawEmail = (req.headers["x-admin-email"] || req.headers["authorization"]?.replace(/^Bearer\s+/i, "") || req.body?.adminEmail || req.query?.adminEmail || "").toString().toLowerCase().trim();
  ...
  app.post("/api/admin/services/:id/toggle-status", requireAdminAuth, async (req, res) => {
    ...
    try {
      if (supabaseServer) {
        await supabaseServer.from("services").update({ available: Boolean(available) }).eq("id", id);
      }
  ...
  app.post("/api/admin/services/:id/feature", requireAdminAuth, async (req, res) => {
    ...
    try {
      if (supabaseServer) {
        await supabaseServer.from("services").update({ is_featured: Boolean(isFeatured) }).eq("id", id);
      }
      adminAuditLogs.unshift({
        id: `log-${Date.now()}`,
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  app.post("/api/admin/services/:id/verify", requireAdminAuth, async (req, res) => {
    ...
    try {
      if (supabaseServer) {
        await supabaseServer.from("services").update({ is_provider_verified: Boolean(isVerified) }).eq("id", id);
      }
      adminAuditLogs.unshift({
        id: `log-${Date.now()}`,
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  app.post("/api/admin/users/:id/toggle-verify", requireAdminAuth, async (req, res) => {
    ...
    try {
      if (supabaseServer) {
        await supabaseServer.from("profiles").update({ is_verified: Boolean(isVerified) }).eq("id", id);
      }
      adminAuditLogs.unshift({
        id: `log-${Date.now()}`,
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  app.post("/api/admin/users/:id/toggle-ban", requireAdminAuth, async (req, res) => {
    ...
    try {
      if (supabaseServer) {
        await supabaseServer.from("profiles").update({ is_banned: Boolean(isBanned) }).eq("id", id);
      }
      adminAuditLogs.unshift({
        id: `log-${Date.now()}`,
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  app.get("/api/admin/requests", requireAdminAuth, async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseServer.from("needs").select("*").order("created_at", { ascending: false });
      if (error) throw error;
...
Respond ONLY with valid JSON adhering to this exact format:
{
  ...
  async function resolveServiceMeta(serviceId, canonicalOrigin) {
    ...
    if (supabaseServer) {
      try {
        const { data, error } = await supabaseServer.from("services").select("id, title, description, images, starting_price, unit, provider_name, service_area, location, available").eq("id", serviceId).maybeSingle();
        if (data && !error && data.available !== false) {
          ...
          description = data.description ? `${data.description.slice(0, 140)}... Available in ${locationStr}${priceStr}. Book verified services on Needly.` : `Book ${data.title} by ${data.provider_name} in ${locationStr} on Needly.`;
  ...
  function injectOpenGraphTags(rawHtml, meta) {
    let html = rawHtml;
  ...
  app.get("/api/public/service/:serviceId", async (req, res) => {
    try {
      ...
      if (supabaseServer) {
        const { data, error } = await supabaseServer.from("services").select("id, title, category, subcategory, description, images, provider_id, provider_name, provider_avatar, provider_rating, provider_review_count, is_provider_verified, starting_price, unit, service_area, location, distance_km, available, skills, packages, jobs_completed, created_at").eq("id", serviceId).maybeSingle();
        if (data && !error) {
  ...
  if (!isProduction) {
    ...
    app.get("/service/:serviceId", async (req, res, next) => {
      try {
        ...
        if (!import_fs.default.existsSync(indexPath)) return next();
        let rawHtml = import_fs.default.readFileSync(indexPath, "utf-8");
    ...
    const distPath = import_fs.default.existsSync(import_path.default.join(process.cwd(), "dist", "index.html")) ? import_path.default.join(process.cwd(), "dist") : import_path.default.join(process.cwd());
    app.get("/service/:serviceId", async (req, res, next) => {
      try {
        const serviceId = req.params.serviceId;
        const origin = `${req.protocol}://${req.get("host")}`;
